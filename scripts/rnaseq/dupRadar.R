...
write.table(dm, file=paste("test/dupRadar/", sample, "_dm", sep=""), sep="\\t")
...
cat("# dupRadar model params\\n", file=paste("test/dupRadar/", sample, "_model.txt", sep=""))
write.table(df, file=paste(sep="", "test/dupRadar/", sample, "_model.txt"), sep="\\t", append=TRUE, row.names=FALSE)
...
write.table(
  ...
  file=paste("test/dupRadar/", sample, "_curve.txt", sep="")
