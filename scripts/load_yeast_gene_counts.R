...
gtf2 <- read.table('/home/ubuntu/hussainather/seqacademy/data/Saccharomyces_cerevisiae.R64-1-1.84.gtf', header = FALSE, sep = '\t',stringsAsFactors=FALSE)
...
save(YeastGeneAnnotation, file='/home/ubuntu/SteveSemick/Yeast_RNAseq_Project/yeastGeneAnnotation.rda')

## Load yeast gene counts

countFileDirectory = '/home/ubuntu/hussainather/seqacademy/test' #Directory that the count files are stored in
...
expInfo = as.data.frame( data.table::fread('/home/ubuntu/hussainather/seqacademy/data/RNASeqSRA.tsv') )
head(expInfo)
...
save(YeastGeneCounts, expInfo, file='/home/ubuntu/SteveSemick/Yeast_RNAseq_Project/YeastGeneCounts.rda')



CountsPCA = prcomp( t(log2(YeastGeneCounts+1) ) )$x

pdf('/home/ubuntu/SteveSemick/test/pca.pdf',height=8,width=8)
par(mar=c(5,6,4,1)+.5)
